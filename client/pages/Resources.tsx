...
const schemes = [
  {
    ...
    process: "Apply online via the official PMAY portal using Aadhaar and income proof.",
  ...
  },
  {
    ...
    process: "Benefits are distributed through the Public Distribution System (ration cards) via State food departments.",
  ...
  },
  {
    ...
    eligibility: ["Indian citizens aged 10+", "No existing bank account required"],
    process: "Open an account at any bank branch or apply via the PMJDY portal using valid ID proof.",
  ...
  },
  {
    ...
    eligibility: ["Small and marginal landholding farmers", "Aadhaar-linked bank account"],
    process: "Register on the PM-KISAN portal with Aadhaar and bank details to receive transfers.",
  ...
  },
  {
    ...
    process: "Apply through LIC of India or authorized agents; documentation includes ID and KYC.",
...
export default function Resources() {
  ...
  const filtered = useMemo(() => {
    return schemes.filter((s) => {
      const matchesQuery = (s.title + s.desc + s.category).toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <section className="container py-8">
      ...
      <div className="mb-6 flex flex-col md:flex-row items-center gap-3">
        ...
        />
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="h-10 rounded-md border px-3">
          {categories.map((c) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {filtered.map((s) => (
          <div key={s.id} className="bg-white rounded-md shadow p-6 flex flex-col">
            ...
            <div className="mt-auto">
              <a href={s.url} target="_blank" rel="noreferrer" className="inline-block w-full text-center px-4 py-2 bg-primary text-white rounded shadow-sm hover:opacity-95">Apply Now</a>
            </div>

            <div className="mt-3 text-xs text-muted-foreground">Category: <span className="font-medium">{s.category}</span></div>
          </div>
        ))}
      </div>
