...
export default function Header() {
  ...
  const headerClass = isHome ? "sticky top-0 z-50 bg-black/40 backdrop-blur-sm" : "sticky top-0 z-50 bg-white/90 backdrop-blur border-b";
      ...
      <div className="container flex items-center justify-between h-14">
        <Link to="/" className="flex items-center gap-2 text-xl font-semibold">
          <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full shadow-md ${isHome ? 'bg-primary text-white' : 'bg-white text-black border'}`}>
            <Search size={16} className={isHome ? 'text-white' : 'text-black'} />
          </span>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {[
            ...
            <NavLink
              key={item.to}
              to={item.to}
              className={({ isActive }) => `${navLinkBase} ${isActive ? "bg-primary text-white" : navDefault}`}
            >
              {item.label}
            </NavLink>
          ))}
          <NavLink to="/login" className={({ isActive }) => `${navLinkBase} ${isActive ? "bg-primary text-white" : navDefault}`}>
            Login
          </NavLink>
        </nav>
      </div>
